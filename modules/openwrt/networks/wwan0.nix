    ...
    "wwan/lte_fallback".text = ''
      ...
      is_5g_station_on() {
        uqmi --device "$DEVICE" --get-cell-location-info | \
          grep -q -e '"physical_cell_id": 282' -e '"physical_cell_id": 295' -e '"physical_cell_id": 296'
      }
      if is_time_close; then
        ...
        logger -t "lte_fallback" "cell 282/295/296 for channel 40936 is online, switching to default"
